...
//					console.log(paragraph.content.substring(paragraph.content.substring(0, nodes[node].startOffset).lastIndexOf(' ') + 1, nodes[node].startOffset))
					...
					if(paragraph.content.substring(nodes[node].startOffset, nodes[node].endOffset) == termData[w].value) {
						entityMatches.push({path: nodes[node].path, startOffset: nodes[node].startOffset, endOffset: nodes[node].endOffset, term: termData[w]});
					}
					if(paragraph.content.substring(paragraph.content.substring(0, nodes[node].startOffset).lastIndexOf(' ') + 1, nodes[node].startOffset) == termData[w].value) {
						...
						entityMatches.push({path: [paragraph.id, 'content'], startOffset: paragraph.content.substring(0, nodes[node].startOffset).lastIndexOf(' ') + 1, endOffset: nodes[node].startOffset, term: termData[w]})
					}
				}
			}
            ...
            if (node.startsWith('paragraph') ) {
				...
				for (let w in termData) {
                    ...
                    while (hits = re.exec(paragraphNode.content)) {
						...
						entityMatches.push({path: [paragraphNode.id, 'content'], startOffset: startOffset, endOffset: endOffset, term: term})
					}
				}
