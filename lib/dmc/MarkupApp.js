...
//					console.log(paragraph.content.substring(paragraph.content.substring(0, nodes[node].startOffset).lastIndexOf(' ') + 1, nodes[node].startOffset))
				   // Handle the superscript	
					...
					if(paragraph.content.substring(paragraph.content.substring(0, nodes[node].startOffset).lastIndexOf(' ') + 1, nodes[node].startOffset) == termData[w].value) {
						entityMatches.push({path: [paragraph.id, 'content'], startOffset: nodes[node].startOffset - termData[w].value.length, endOffset: nodes[node].startOffset, term: termData[w]})
					}
				}
			}
        ...
        for (let node in nodes) {
            if (node.startsWith('paragraph') ) {
				...
				for(let entityMatch in entityMatches) {
						...
						let paragraphNode = window.doc.get(entityMatches[entityMatch].path[0])

						//if(entityMatches[entityMatch].type == "superscript") {
					
						//	paragraphNode.content = paragraphNode.content.substring(0, startOffset) + " " + paragraphNode.content.substring(startOffset, paragraphNode.content.length)
						...
						for(let node in subscriptNodes) {
							if(subscriptNodes[node].startOffset > startOffset && subscriptNodes[node].startOffset < endOffset) {
								console.log(startOffset + " " + endOffset + " " + subscriptNodes[node].startOffset + " " + subscriptNodes[node].endOffset)
...
//								paragraphNode.content = paragraphNode.content.substring(0, endOffset) + " " + paragraphNode.content.substring(endOffset, paragraphNode.content.length)

//								console.log (paragraphNode.content.substring(endOffset, paragraphNode.content.length))
...
//								paragraphNode.content = paragraphNode.content.substring(0, startOffset - 1) + paragraphNode.content.substring(startOffset, paragraphNode.content.length)
								documentSession.transaction(function(tx, args) {
...
//								paragraphNode.content = paragraphNode.content.substring(0, endOffset) + paragraphNode.content.substring(endOffset, paragraphNode.content.length)
						...
						//	if(entityMatches[entityMatch].type == "superscript") {
						//	//	console.log(paragraphNode.content.substring(0, startOffset))
						//		console.log(paragraphNode)
						//		paragraphNode.content = paragraphNode.content.substring(0, startOffset - 1) + paragraphNode.content.substring(startOffset, paragraphNode.content.length)
						//	}
							...
							// Save ext-link ID
							term.extLinkId = res.anno.id;
                            // term.selection.extLinkId['xlink:href'] = term.lexica[0].link ;
                            // from ExtLinkJs and https://jats.nlm.nih.gov/archiving/tag-library/1.1/element/ext-link.html
                           // selectionState.getAnnotationsForType("ext-link")[0].attributes['xlink:href'] = term.lexica[0].link ;
							// term.selection.extLinkId.hrefLink = {};
							// term.selection.extLinkId.hrefLink.set(term.lexica[0].link);
							// Set the href link in the tag
							// selectionState.getAnnotationsForType("ext-link")[0].hrefLink.set(term.lexica[0].link);
							documentSession.transaction(function(tx, args) {
								tx.set([res.anno.id, 'hrefLink'], term.lexica[0].link)
								tx.set([res.anno.id, 'entityType'], "mu")

							})
						//	res.anno.attributes['xlink:href'] = term.lexica[0].link;
//							console.log(res.anno)
